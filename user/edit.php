...
if (isset($_GET['id'])) {
    $user_id = (int) $_GET['id'];
    $user_stmt = $mysqli->prepare("SELECT u.username, u.image_url, up.first_name, up.last_name, up.dob, up.gender, up.blood_type, uc.telegram_chat_id, r.name as role_name FROM users u JOIN user_profiles up ON u.id = up.user_id LEFT JOIN user_contacts uc ON u.id = uc.user_id JOIN roles r ON u.role_id = r.id WHERE u.id = ?");
    $user_stmt->bind_param("i", $user_id);
    $user_stmt->execute();
    $user_stmt->store_result();
    $user_stmt->bind_result($username, $image_url, $first_name, $last_name, $dob, $gender, $blood_type, $telegram_chat_id, $role_name);
    $user_stmt->fetch();
    $user_stmt->close();
